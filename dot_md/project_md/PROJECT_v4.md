...
├── Provider reliability/timeout hardening            🔧
└── Retrieval + latency regression validation         🔧
│
▼
NEXT
│
...
The project is currently in a **retrieval stabilization and optimization checkpoint immediately before the Agentic phase**.
...
The retrieval architecture is now:
...
The deduplication step and preservation of separate RRF/cross-encoder diagnostics are the **next implementation checkpoint**. These are deliberately query-time changes and should not modify the working ingestion pipeline.

### Next major implementation phase

After the optimized RAG baseline is stable:
...
The project is now in a **retrieval stabilization and optimization checkpoint**. The objective is to improve retrieval quality and efficiency without breaking the working ingestion pipeline or the existing RAG graph contract.

```text
1. Preserve working ingestion
       ↓
2. Verify document-scoped retrieval
       ↓
...
6. Cross-encoder reranking
       ↓
7. Preserve retrieval diagnostics
       ↓
8. Calibrate retrieval-confidence thresholds
       ↓
9. Make relevance grading conditional
       ↓
10. Reduce unnecessary rewrite/re-retrieval cycles
       ↓
11. Optimize generation context size
       ↓
12. Measure quality + latency + LLM call count
       ↓
13. Freeze optimized RAG baseline
       ↓
14. Build the agent/tool layer
```
...
## 34.1 Current Retrieval Architecture — Updated

The retrieval layer has evolved beyond dense-only search:

```text
User query
    ↓
document-scoped filters
    ↓
Dense retrieval + BM25 / sparse retrieval
    ↓
RRF fusion
    ↓
query-time logical deduplication
    ↓
cross-encoder reranking
    ↓
overview + content
...
The current vectorstore uses the raw Pinecone SDK for hybrid operations so each chunk can carry both dense and sparse BM25 vectors. The sparse encoder is document-specific.

### RRF fusion

Dense and BM25 results are retrieved separately and combined with **Reciprocal Rank Fusion (RRF)** because their raw score scales are not directly comparable. The implementation preserves dense rank/score, BM25 rank/score, and RRF score for diagnostics.

### Candidate deduplication — CURRENT CHECKPOINT

Recent retrieval logs exposed duplicate logical chunks reaching the cross-encoder. The next query-time change is therefore:

```text
hybrid candidates
       ↓
deduplicate logical chunks
       ↓
cross-encoder
```

The planned identity is based on:

```text
document_id + chunk type + page + normalized content hash
```

The first ranked occurrence is retained. This does **not** modify ingestion, chunking, BM25 fitting, Pinecone vectors, or the document registry.
...
It receives `(query, passage)` pairs, produces a relevance logit, and applies sigmoid explicitly for a normalized diagnostic score. The current returned score remains the cross-encoder score so the downstream graph contract is unchanged.
...
The old retrieval thresholds must not be assumed valid after reranking. The current `retrieval_scores` represent cross-encoder relevance scores rather than the old vector-similarity scale. Retrieval-confidence thresholds therefore require calibration against the current retrieval path.

## 34.2 Retrieval Validation Checkpoint

Use these minimum regression queries:

```text
1. Tell me about Problem Decomposition.
2. Is this part of a bigger topic?
```

Inspect the trace from:
...
Verify duplicate removal, document scoping, dense/BM25 contribution, RRF diagnostics, cross-encoder ordering, and downstream compatibility. Do not change adaptive retrieval thresholds until this checkpoint passes.

## 34.3 What Is NOT Being Changed

This checkpoint does not change:

```text
PDF/image/audio ingestion
chunking
overview generation
BM25 document fitting
Pinecone upsert
document registry
document_id generation
FastAPI ingestion flow
Streamlit ingestion flow
PostgreSQL schema
```

> **Change the retrieval boundary before changing the ingestion foundation.**
...
# 36. Document-Scoped Retrieval — Latest Change (2026-08-12)

## Problem that triggered this change

Performance tracing showed a representative request performing:

```text
retrieve × 3 graph iterations
2 Pinecone searches per iteration
= 6 vector searches
15.50 seconds spent in retrieval
```

The retrieval trace also showed chunks from multiple unrelated documents being returned for a question about a single document.

The underlying cause was that content retrieval was effectively global:
...
while only overview retrieval had an explicit `type=overview` filter.

This could cause:

- mixed-document context;
- low relevance grades;
- unnecessary query rewrites;
- repeated re-retrieval;
- increased latency;
- unnecessary LLM calls.

## Design decision

We introduced a stable `document_id` and made it part of the retrieval state.

The document ID is generated from the exact file contents using SHA-256:
...
This was chosen over using the temporary upload path because the application previously used temporary files such as:

```text
C:\Users\...\AppData\Local\Temp\tmp....pdf
```

Temporary paths are implementation details, not durable document identities.

A content-derived ID also means the same exact file receives the same ID when uploaded again.

## Metadata propagation

Every content chunk now carries:
...
Existing loader metadata such as page information is preserved.

This allows Pinecone to distinguish:
...
instead of treating all chunks as one undifferentiated search space.

## State propagation

`RAGState` now contains:

```python
document_id
```

The value travels through the LangGraph request so retrieval can use the active document scope.

## Retrieval behavior

When `document_id` is available:
...
```text
             ...
             ┌──────────┴──────────┐
             ▼                     ▼
        overview search       content search
             │                     │
       document_id +          document_id +
       type=overview          type=content
             │                     │
             └──────────┬──────────┘
                        ▼
                 scoped results
...
The existing vectorstore API already accepted metadata filters, so `vectorstore.py` did not require a redesign.

A temporary backward-compatible global fallback remains when no `document_id` is supplied. The application UI/API are being updated so normal document questions provide the active document ID.

## API changes

`/ingest` now exposes the generated `document_id`.
...
The document ID is passed into the LangGraph initial state.

## Streamlit changes

The Streamlit UI now:

- loads the registered documents;
- maintains an active document ID;
- displays an active-document selector;
- automatically selects a newly ingested document when appropriate;
- sends `document_id` with every query;
- prevents document-scoped questions when no active document exists;
- resets the conversation thread when the user switches documents.

The thread reset is intentional: a conversation about Document A should not carry conversational state into Document B.

## Registry changes

The PostgreSQL document registry now stores:
...
The schema migration uses `ADD COLUMN IF NOT EXISTS` so the development database can evolve without manually dropping the existing table.

## Why we chose this architecture

### Why `document_id` instead of filename?

Filenames are user-facing labels and are not guaranteed to be unique.

### Why SHA-256 instead of a random UUID?

A random UUID identifies an ingestion event.

A content hash identifies the document content itself.

For this project, deterministic identity is more useful because the same file can be recognized across uploads.

### Why store the ID in both PostgreSQL and Pinecone?

PostgreSQL acts as the application-level document registry.

Pinecone needs the ID as vector metadata for retrieval filtering.

The shared identifier connects the two layers.

### Why filter both overview and content?

Filtering only content would still allow unrelated overview chunks into the result set.

Both retrieval paths therefore receive the same document scope.

### Why not redesign the vectorstore?

The existing `retrieve_with_scores()` already supports metadata filters. Reusing that capability keeps the change small and reduces regression risk.

## Migration consequence

Vectors ingested before this change do not have `document_id`.

Therefore, existing vectors should not be assumed to work with document-scoped filtering.
...
verify Pinecone metadata
    ↓
query using document_id
    ↓
verify all retrieved sources belong to selected document
    ↓
benchmark latency and CRAG retries
```

A development index rebuild/re-ingestion may therefore be required.

## Expected impact

The goal is not merely to make Pinecone faster.
...
better retrieval precision
        ↓
fewer irrelevant grades
        ↓
fewer rewrites
        ↓
fewer re-retrieval cycles
        ↓
fewer LLM calls
        ↓
...
This is aligned with the project's optimization rule:
...
A speed improvement that reduces answer quality is not considered a successful optimization.

---

# 37. Implementation Files Changed in This Milestone

The following layers were modified:

```text
ingestion/chunking.py
    → propagate document_id and user-facing filename

ingestion/registry.py
    → persist document_id

ingestion/pipeline.py
    → generate deterministic SHA-256 document_id
    → attach ID to chunks and overview
    → record ID in registry

graph/state.py
    → carry document_id through LangGraph

graph/nodes.py
    → apply document-scoped Pinecone filters

api/main.py
    → accept document_id on /query
    → return document_id from /ingest

streamlit_app.py
    → active document selection
    → send document_id with queries
    → reset session when document changes
```

`vectorstore.py` was intentionally left unchanged because its existing metadata-filter interface was sufficient.

---

# 38. Current Architecture After Document Scoping
...
```text
        ...
        POST /ingest         POST /query
          ...
          FastAPI             FastAPI
             │                   │
             ▼                   ▼
        ingest_file          document_id
             │                   │
             ▼                   ▼
       SHA-256 ID           LangGraph
             │                   │
             ▼                   ▼
     chunks + overview       retrieve()
             │                   │
             ▼                   ▼
          Pinecone       metadata-filtered search
             │                   │
             └─────────┬─────────┘
                       ▼
                 document-scoped
...
This moves the system one step closer to a reliable multi-document RAG foundation for the future agentic layer.

---

# 39. New Development Principle

The recent debugging cycle reinforced an important engineering principle:

> **Do not optimize downstream reasoning until the retrieval boundary is correct.**

We initially observed expensive CRAG retries and considered reducing LLM calls.

Tracing revealed that retrieval itself was operating over a mixed-document search space.

Therefore the correct sequence became:
...
This prevents us from optimizing symptoms instead of root causes.

---

# 40. Retrieval Optimization Checkpoint — 2026-08-19

Recent hybrid retrieval logs showed duplicate logical chunks reaching the cross-encoder. The root issue is that unique Pinecone vector IDs do not guarantee unique logical chunks.

## Decision

Keep stored vectors and ingestion unchanged. Apply query-time deduplication after RRF and before cross-encoder reranking:
...
The deduplication identity should use:

```text
document_id + type + page + normalized content hash
```
...
Do not change the downstream retrieval-score contract during this checkpoint.

## Files for the next implementation checkpoint

```text
src/agentic_rag/retrieval/reranker.py
src/agentic_rag/graph/nodes.py
```

`config.py` should only change if calibration/configuration values require it after validation. Do not modify `src/agentic_rag/ingestion/*`, stored Pinecone vectors, BM25 fitting, or the ingestion API for this checkpoint.
...
```text
Tell me about Problem Decomposition.
Is this part of a bigger topic?
```

Inspect hybrid retrieval, RRF, deduplication, reranking, retrieval assessment, and performance summary before moving to threshold calibration.

---

# 41. Latest Handoff State

For the next development session:

1. Read `PROJECT.md` first.
2. Treat current source code as authoritative.
3. The immediate task is retrieval stabilization, not agent-tool implementation.
4. Hybrid dense + BM25 retrieval, RRF fusion, and cross-encoder reranking are already implemented.
5. Duplicate logical candidates were observed in reranking logs.
6. The next code change is query-time candidate deduplication.
7. Preserve RRF and cross-encoder diagnostics separately.
8. Do not recalibrate retrieval thresholds until the candidate flow is validated.
9. Do not touch ingestion for this retrieval checkpoint.
10. Do not remove CRAG.
11. After the optimized RAG baseline is stable, continue toward `RAG tool → calculator → web search → tool selection → execution loop`.

The system remains **Corrective / Advanced RAG**, not yet a true tool-using agent.

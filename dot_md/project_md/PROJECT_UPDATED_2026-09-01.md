...
# 55. GPU / PyTorch Environment Finding — 2026-09-01
...
This confirms that the application is using a CPU-only PyTorch build. The earlier CPU model-selection messages are therefore symptoms of the environment, not evidence that application device selection is inherently broken.

The environment is being corrected before changing embedding/reranker device-selection code. After installation, verify CUDA availability and actual tensor allocation before trusting GPU performance measurements.
...
- Initial logging test passed.
- CPU-only PyTorch environment diagnosed.
- NVIDIA GPU and driver confirmed operational at the system level.

## Pending

- Live end-to-end validation of the final request-scoped diagnostic lifecycle.
- Final cleanup of Uvicorn / HTTPX / Hugging Face startup noise.
- CUDA-enabled PyTorch verification.
- Explicit CUDA verification for embedding and cross-encoder models.
...
```text
1. Complete CUDA-enabled PyTorch installation
        ↓
2. Verify torch.cuda.is_available()
        ↓
3. Verify RTX 3060 tensor allocation
        ↓
4. Verify embedding model device
        ↓
5. Verify cross-encoder device
        ↓
6. Clean third-party console noise
        ↓
7. Run one clean RAG query
        ↓
8. Inspect JSONL + JSON
        ↓
9. Instrument retrieval pipeline
        ↓
10. Begin evidence-driven retrieval investigation
```

No retrieval thresholds should be changed merely because the logging work is in progress.
...
The current investigation is focused on making the RAG pipeline observable without polluting persistent LangGraph state. The previous `trace` accumulation mechanism has been removed from the current node-return path, and request-scoped performance/diagnostic infrastructure is being established outside `RAGState`. The `rewrite_query` function has been corrected so every corrective rewrite remains anchored to the original user question, preventing information loss across repeated rewrites. Structured JSONL/JSON diagnostics have passed an isolated test. The latest environment investigation confirmed that the RTX 3060 and NVIDIA driver are available, but the active Python environment contains the CPU-only `torch 2.13.0+cpu` build; CUDA must therefore be restored at the environment level before GPU performance measurements are trusted. The next objective is to establish a clean request-scoped observability baseline, verify CUDA execution for the embedding and cross-encoder components, reduce third-party console noise without sacrificing diagnostic information, and only then instrument and optimize the retrieval pipeline.
